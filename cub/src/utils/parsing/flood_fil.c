...
/*   Updated: 2026/03/14 10:54:17 by tarandri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	process_point(char **map, t_point *stack, int *top, int stack_size,
...
int	flood_fill(char **map, int start_x, int start_y, int width, int height)
{
	...
	while (top > 0)
	{
		if (process_point(map, stack, &top, stack_size, dx, dy, width,
				height) == -1)
			return (free(stack), 0);
	}
	free(stack);
